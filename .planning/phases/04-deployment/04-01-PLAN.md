...
- src/components/BookingBlock.jsx (Phase 3 gap closure - timezone + position fix)
- src/components/BookingBlock.css (Phase 3 gap closure - opacity)
- src/components/BookingOverlay.jsx (Phase 3 gap closure - threading useNSWTime)
- src/components/WeekBookingBlock.jsx (Phase 3 gap closure - timezone)
- src/components/WeekDayOverlay.jsx (Phase 3 gap closure - threading useNSWTime)
- src/hooks/useKeyboard.js (Phase 3 gap closure - B and T hotkeys)
</context>
...
**Copy all changed files from template to eureka (15 files):**
```bash
# Utils
...
cp "$TEMPLATE/src/components/BookingBlock.jsx" "$EUREKA/src/components/BookingBlock.jsx"
cp "$TEMPLATE/src/components/BookingBlock.css" "$EUREKA/src/components/BookingBlock.css"
cp "$TEMPLATE/src/components/BookingOverlay.jsx" "$EUREKA/src/components/BookingOverlay.jsx"

# Week View
cp "$TEMPLATE/src/components/WeekView.jsx" "$EUREKA/src/components/WeekView.jsx"
cp "$TEMPLATE/src/components/WeekBookingBlock.jsx" "$EUREKA/src/components/WeekBookingBlock.jsx"
cp "$TEMPLATE/src/components/WeekDayOverlay.jsx" "$EUREKA/src/components/WeekDayOverlay.jsx"

# Hooks
cp "$TEMPLATE/src/hooks/useKeyboard.js" "$EUREKA/src/hooks/useKeyboard.js"
```
...
- Add Book Now button for instant current-hour booking [B hotkey]
- Add NSW timezone toggle with DST detection [T hotkey]
- Fix booking block position alignment and opacity
...
Should return 200.
  ...
  <verify>
All 15 files copied. `git push` succeeds for eureka repo. Curl returns 200.
  </verify>
  <done>
...
4. **Hotkeys:** Press B key (opens Book Now panel), press T key (toggles timezone)
5. **Booking Display:** Toggle timezone and verify booking card times update (e.g., "9-10 AM" → "10-11 AM")
6. Refresh page - timezone toggle state persists

**Test on eureka (booking-eureka.vercel.app):**
1. Same features work identically
2. Timezone toggle persists across refresh
3. Booking blocks align correctly with slots (no visual overlap)
...
3. Eureka repo: `git push` completed without errors (15 files synced)
4. Eureka instance: Returns HTTP 200
5. Feature verification: Time slot fix, Book Now, timezone toggle, hotkeys work on all instances
6. Visual verification: Booking blocks align correctly (no overlap with slot labels)
7. Persistence: Timezone preference persists in localStorage on each instance
</verification>

<success_criteria>
- DEPL-01: Changes pushed to template repo, 4 admin instances auto-deployed
- DEPL-02: 15 changed files synced to booking-eureka repo and pushed
- All 5 instances show: time slot fix, Book Now button [B], timezone toggle [T], hotkeys
- Booking display: Time ranges update when timezone toggled
- No regressions: existing bookings still work, no visual overlap issues
</success_criteria>

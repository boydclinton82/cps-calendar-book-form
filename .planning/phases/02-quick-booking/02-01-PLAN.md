...
@.planning/phases/01-time-slot-fix/01-01-SUMMARY.md
@src/App.jsx
@src/components/Header.jsx
@src/components/Header.css
@src/utils/time.js
...
**Phase 1 Context (IMPORTANT):**
`isSlotPast()` in `src/utils/time.js` was updated to check slot END time (hour + 1):
```javascript
export function isSlotPast(date, hour) {
  const now = new Date();
  const slotEnd = new Date(date);
  slotEnd.setHours(hour + 1, 0, 0, 0);  // END of hour
  return slotEnd <= now;  // True only when next hour begins
}
```
At 10:15, `isSlotPast(now, 10)` returns FALSE - the 10:00 slot is available until 11:00.
This is the foundation for "Book Now" working at any minute during the current hour.
</context>

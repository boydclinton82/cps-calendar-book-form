...
8. Update function signature to accept new props:
```javascript
...
9. Add timezone toggle button in header-actions div, AFTER the week-toggle button:
```jsx
<button
...
11. In mobile breakpoint (@media max-width: 600px), show timezone toggle but perhaps make it smaller:
```css
